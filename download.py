        ...
        for air in args.air:
            ...
            if air_dict.get("format") is None:
                model_data = get_model_data(
                    air_dict.get("version") or air_dict.get("id"), token
                )
                air_dict["format"] = model_data["format"]
                air_dict["type"] = model_data["type"]
                print(f"Found AIR: {air} (format: {model_data})")
            if (
                not args.force_unsafe
                and air_dict.get("format", "").lower() != "safetensor"
            ):
                print(
                    f"❌ Refusing unsafe format in AIR: {air} (found: {air_dict.get('format')})"
                )
                continue
